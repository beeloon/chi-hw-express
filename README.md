...
## Questions
1. Что делать в сервисах?
2. Правильно ли указаны пути для создания поста (/api/users/:id) и для поиска поста по айди автора (/api/posts/user/:authorId)
3. Исправить работу update метода:
   - Он должен корректно обновлять только те свойства которые передаются
   - Не должен перезаписывать свойства authorId  в посте